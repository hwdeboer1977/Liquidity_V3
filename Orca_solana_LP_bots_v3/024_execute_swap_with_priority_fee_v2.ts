...
async function main() {
  ...
  // Amount_in = 0.3 then Swap 0.0003 SOL for USDC
  // Amount_in = 300 then Swap 0.3 SOL for USDC
  const amount_in = new Decimal("0.3" /* SOL */);
  ...
  console.log(
    "estimatedAmountIn:",
    DecimalUtil.fromBN(quote.estimatedAmountIn, SOL.decimals).toString(),
    "USDC"
  );
  console.log(
    "estimatedAmountOut:",
    DecimalUtil.fromBN(quote.estimatedAmountOut, USDC.decimals).toString(),
    "SOL"
  );
  console.log(
    "otherAmountThreshold:",
    DecimalUtil.fromBN(quote.otherAmountThreshold, USDC.decimals).toString(),
    "SOL"
  );
  ...
  const set_compute_unit_price_ix = ComputeBudgetProgram.setComputeUnitPrice({
    ...
    microLamports: Math.floor(
      (additional_fee_in_lamports * 10_000_000) / estimated_compute_units
    ),
  });
